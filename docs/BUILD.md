...
- **Web:** Rust target `wasm32-unknown-emscripten`, Emscripten SDK, and **`GODOT4_BIN`** pointing to the V-Sekai world-godot editor (see **API generation** above) so gdext can generate bindings. Build with `cargo build --release --target wasm32-unknown-emscripten --no-default-features`. **Windows:** use `.\misc\build-wasm.ps1` (scoop emscripten at `%USERPROFILE%\scoop\apps\emscripten\current`); load env first with `& "%USERPROFILE%\scoop\apps\emscripten\current\emsdk_env.ps1"` if you need emcc in the same shell. Godot export templates must be built with `dlink_enabled=yes` for GDExtension on web. **Note:** as of 2025, the **ort** crate 2.0.0-rc.11 may fail to build for wasm (SmallVec API mismatch); CI web job uses `continue-on-error`.
