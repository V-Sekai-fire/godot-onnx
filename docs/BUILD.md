...
- **Web:** Rust target `wasm32-unknown-emscripten`, Emscripten SDK, and **`GODOT4_BIN`** pointing to the V-Sekai world-godot editor (see **API generation** above) so gdext can generate bindings. Build with `cargo build --release --target wasm32-unknown-emscripten --no-default-features`. **Windows:** load Emscripten with `& "%USERPROFILE%\scoop\apps\emscripten\current\emsdk_env.ps1"` if you need emcc. Godot export templates must be built with `dlink_enabled=yes` for GDExtension on web. **Backend:** wasm uses **ort-tract** (pure Rust); `ort` is patched to git `main` so the wasm build succeeds (see [docs/WEB_BACKENDS.md](WEB_BACKENDS.md)).
